...
I0 <- data_india[start_point,3]-data_india[start_point-7,3]
propA0 <- 1   
A0 <- I0 * propA0
...
####estimate
#####
myfix_pars <- c(a = 1, De = 5.1, delta=h_r/7 , gamma = (1-0.01)/22.3, eta = 0.85/22.3, omega=0.01/15.4, nu=0.15/15.4  , N = N_india)
############
...
pars_estimate_india_2 <- fun_mcmc(init_pars = c(1.6,0.2), step_pars =  c(1.6,0.2)/ 100, init_state_num = yinit, observed_num = india, niter = 1000000, BurnIn = 200000, trace_num = 100, runMLE = T)
mcmc_pars_estimate_india <- round(pars_estimate_india_2$mcmc_estimates, 3)
write.table(mcmc_pars_estimate_india, "mcmc_pars_estimate_india_2.txt", quote = F, row.names = F, sep = "\t")
