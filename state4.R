...
start_point=which(data_state2$Date==as.Date('2020/03/15'))
...
pars_estimate_state <- fun_mcmc(init_pars = c(1.6,0.4,0.4,0.4,0.2,0.2,0.2,0.2), step_pars =  c(1.6,0.4,0.4,0.4,0.2,0.2,0.2,0.2)/ 100, init_state_num = yinit, observed_num = state, niter = 1000000, BurnIn = 200000, trace_num = 100, runMLE = T)
mcmc_pars_estimate_state <- round(pars_estimate_state$mcmc_estimates, 3)
write.table(mcmc_pars_estimate_state, paste0("mcmc_pars_estimate_",indicator[i],".txt"), quote = F, row.names = F, sep = "\t")
