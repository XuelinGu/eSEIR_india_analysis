...
fun_mle <- function(init_pars, init_state_num, observed_num, opt_num) {
  ...
  colnames(result_mat) <- c("likelihood", "convergence","b1","b2","b3","b4","r1","r2","r3","r4","b1_sd","b2_sd","b3_sd","b4_sd","r1_sd","r2_sd","r3_sd","r4_sd")
  result_mat[, 1] <- -Inf
  for(i in 1:opt_num) {
    passed <- FALSE
    while (!passed) {
...
fun_R0 <- function(estpar,myfix_pars) {
  ...
  b <- estpar[2]
  r <- estpar[6]
  R02 <- (1-r)*(22.3^2*gamma+15.4^2*omega)*a*b+r*b*(h_r*(Dh+eta*22.3^2+nu*15.4^2)+(1-h_r)*(gamma*22.3^2+omega*15.4^2))
  b <- estpar[3]
  r <- estpar[7]
  R03 <- (1-r)*(22.3^2*gamma+15.4^2*omega)*a*b+r*b*(h_r*(Dh+eta*22.3^2+nu*15.4^2)+(1-h_r)*(gamma*22.3^2+omega*15.4^2))
  b <- estpar[4]
  r <- estpar[8]
  R04 <- (1-r)*(22.3^2*gamma+15.4^2*omega)*a*b+r*b*(h_r*(Dh+eta*22.3^2+nu*15.4^2)+(1-h_r)*(gamma*22.3^2+omega*15.4^2))
  
  R0 <- c(mean(R01),mean(R02),mean(R03),mean(R04))
  return(R0)
}
fun_mcmc <- function(init_pars, step_pars, init_state_num, observed_num, niter = 1000000, BurnIn = 200000, trace_num = 100, runMLE = T){
  ...
  np <- length(init_pars)
  pmat <- matrix(0, ((niter + BurnIn) / trace_num) + 1, np+4) ## parameters + R0 for five periods
  colnames(pmat) <- c("b1","b2","b3","b4","r1","r2","r3","r4", "R01", "R02", "R03", "R04")
  pmat[1, 1:np] <- init_pars
  pmat[1, (np+1):(np+4)] <- R0_est
  rm(R0_est, predat)
  ##
  pars_now <- init_pars
  ...
  for(i in 2:(niter + BurnIn)){
    ...
    if(i %% trace_num == 0) {
      ...
      pmat[(i / trace_num) + 1, (np+1):(np+4)] <- R0_est
      rm(R0_est, predat)
    }
  ...
  cat("##              b1  b2  b3 b4  r1  r2  r3 r4 R01  R02  R03  R04       ", fill = T)
...
pars_estimate_india_4 <- fun_mcmc(init_pars = c(1.6,0.4,0.4,0.4,0.2,0.2,0.2,0.2), step_pars =  c(1.6,0.4,0.4,0.4,0.2,0.2,0.2,0.2)/ 100, init_state_num = yinit, observed_num = india, niter = 1000000, BurnIn = 200000, trace_num = 100, runMLE = T)
mcmc_pars_estimate_india_4 <- round(pars_estimate_india_4$mcmc_estimates, 3)
write.table(mcmc_pars_estimate_india_4, "mcmc_pars_estimate_india_4.txt", quote = F, row.names = F, sep = "\t")
#####